...
static inline NSString *cachePath() {
    return [NSHomeDirectory() stringByAppendingPathComponent:@"Documents/ATNetworkingCaches"];
}
...
+ (HYBURLSessionTask *)_requestWithUrl:(NSString *)url
    ...
    if ([self baseUrl] == nil) {
        if ([NSURL URLWithString:url] == nil) {
            ATLog(@"URLString无效，无法生成URL。可能是URL中有中文，请尝试Encode URL");
            return nil;
        }
    } else {
        ...
        if (sg_cacheGet) {
            if (sg_shoulObtainLocalWhenUnconnected) {
                if (sg_networkStatus == kHYBNetworkStatusNotReachable ||  sg_networkStatus == kHYBNetworkStatusUnknown ) {
                    id response = [ATNetworkingManager cahceResponseWithURL:absolute
                                                           parameters:params];
                    if (response) {
                        if (success) {
...
+ (HYBURLSessionTask *)uploadFileWithUrl:(NSString *)url
    ...
    if ([NSURL URLWithString:uploadingFile] == nil) {
        ATLog(@"uploadingFile无效，无法生成URL。请检查待上传文件是否存在");
        return nil;
    }
    ...
    if (uploadURL == nil) {
        ATLog(@"URLString无效，无法生成URL。可能是URL中有中文或特殊字符，请尝试Encode URL");
        return nil;
    }
...
+ (HYBURLSessionTask *)downloadWithUrl:(NSString *)url
    ...
    session = [manager downloadTaskWithRequest:downloadRequest progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        if (error == nil) {
            ...
            if ([self isDebug]) {
                ATLog(@"Download success for url %@",
            ...
            if ([self isDebug]) {
                ATLog(@"Download fail for url %@, reason : %@",
                          [self absoluteUrlWithPath:url],
                          [error description]);
            }
        }
...
+ (id)cahceResponseWithURL:(NSString *)url parameters:params {
    ...
    if (url) {
        ...
        ///////////////////////////////////////////////////////////////////////////////
        // 此处需要删除time字段 (业务需求 每次接口请求中 都会带time值 所以需要删除)
        NSMutableDictionary *mutParams = [NSMutableDictionary dictionaryWithDictionary:params];
        [mutParams removeObjectForKey:@"time"];
        [mutParams removeObjectForKey:@"sign"];
        ///////////////////////////////////////////////////////////////////////////////
        NSString *absoluteURL = [self generateGETAbsoluteURL:url params:mutParams];
        NSString *key = [NSString atNetworking_md5:absoluteURL];
        NSString *path = [directoryPath stringByAppendingPathComponent:key];
        
        NSData *data = [[NSFileManager defaultManager] contentsAtPath:path];
        if (data) {
            cacheData = data;
            ATLog(@"Read data from cache for url: %@\n", url);
        }
    }
...
+ (void)cacheResponseObject:(id)responseObject request:(NSURLRequest *)request parameters:params {
    if (request && responseObject && ![responseObject isKindOfClass:[NSNull class]]) {
        ...
        NSString *absoluteURL = [self generateGETAbsoluteURL:request.URL.absoluteString params:mutParams];
        NSString *key = [NSString atNetworking_md5:absoluteURL];
        NSString *path = [directoryPath stringByAppendingPathComponent:key];
        ...
        if (data && error == nil) {
            ...
            if (isOk) {
                ATLog(@"cache file ok for request: %@\n", absoluteURL);
            } else {
                ATLog(@"cache file error for request: %@\n", absoluteURL);
            }
        }
    }
}
